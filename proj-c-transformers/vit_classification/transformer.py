...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = torch.multiply(query, key.transpose(1, 2))
        scale_para = torch.sqrt(torch.tensor(D, dtype=torch.float32))
        dot_product = dot_product / scale_para
        ...
        dot_dropout = self.dropout(dot_product)
        y = torch.matmul(dot_dropout, value)
        return y
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        if attn_mask is not None:
            ...
            additive_mask = attn_mask.unsqueeze(1).repeat(1, H, 1, 1)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        positional_encoding = self.encoding(positions)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # unk_token_id = self.word_to_idx["<UNK>"]
        # # print(len(self.word_to_idx), scores.shape[-1])
    ...
    def sample(self, features, max_length=30):
        ...
        with torch.no_grad():
            ...
            for t in range(max_length):
                ...
                # unk_token_id = self.word_to_idx["<UNK>"]
                # output_logits[:, unk_token_id] = float('-inf')
